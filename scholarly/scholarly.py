...
def _find_tag_id_name(__data, tag, text):
    elements = __data.find_all(tag)
    for element in elements:
        if 'id' in element.attrs and text in element.attrs['id'][0]:
            return element.attrs['id'][0]
...
def search_author_gsid(gsid):
    ...
    soup = _get_soup(_HOST+url)
    name = soup.select("div #gsc_prf_in")[0].text
    return name
    return search_author(name)
