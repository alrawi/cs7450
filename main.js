...
d3.csv("univ-info.csv", function (data) {
  ...
  d3.json("states.json", function (json) {
    ...
    for (let index = 0; index < radii.length; index++) {
      ...
      svg.append("text")
        .text(legendVal)
        .attr("transform", "translate(1220," + textypos + ")")
    ...
    function collegeClick(college) {
      ...
      // Just draw lines
      d3.json("./links.json", function (link_json) {
        ...
        for (let index = 0; index < source_xy.length; index++) {
          svg.append("line")
            ...
            .style("stroke", "black")
            .style("stroke-width", weights[index])
    ...
    function connectionsOut(college) {
      svg.selectAll('line')
        .remove()
    }
